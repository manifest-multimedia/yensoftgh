...
<body>
        <div class="report-card">
            ...
            <h3 style="text-align: center; text-decoration: upper; text-transform: uppercase;">Statement of Payroll {{ $month }}</h3>
            <hr>
    @if ($contributions->isEmpty())
        <p>No payroll computations found for the selected month and year.</p>
    @else
            ...
            <tbody>
                ...
                @foreach ($contributions as $contribution)
                    <tr>
                        ...
                        <td style="text-align: right;">{{ number_format($contribution->gross_salary, 2) }}</td>
                        <td style="text-align: right;">{{ number_format($contribution->tax_amount + $contribution->employee_contribution + $contribution->other_deductions, 2) }}</td>
                        <td style="text-align: right;">{{ number_format($contribution->net_salary, 2) }}</td>
                    </tr>

                    @php
                        $totalBasicSalary += $contribution->basic_salary;
                        $totalAllownces += $contribution->allowances;
                        $totalGrossSalary += $contribution->gross_salary;
                        $totalDeductions += $contribution->tax_amount + $contribution->employee_contribution + $contribution->other_deductions;
                        $totalNetSalary += $contribution->net_salary;
                    @endphp
                @endforeach
                ...
                <tr>
                    ...
                    <th style="text-align: right;">{{ number_format($totalAllownces, 2) }}</th>
                    <th style="text-align: right;">{{ number_format($totalGrossSalary, 2) }}</th>
                    <th style="text-align: right;">{{ number_format($totalDeductions, 2) }}</th>
                    <th style="text-align: right;">{{ number_format($totalNetSalary, 2) }}</td>
                </tr>
            </tbody>
        </table>
